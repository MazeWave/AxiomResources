...
-- Lighten Brush script 2.0 by MazeWave
...
local function	LightenLinearRGB(rgb, factor)
	...
	if (rgb[1] + scaled_factor > 255) then
		rgb[1] = 255
	end
	if (rgb[2] + scaled_factor > 255) then
		rgb[2] = 255
	end
	if (rgb[3] + scaled_factor > 255) then
		rgb[3] = 255
	end
...
if (getBlock(x, y, z) == blocks.air) then
	...
	return findClosestBlockToRGB(getFinalLightenRGBasHEX(getBlock(x, y, z)), GetFlagsBinary(), index)
end
